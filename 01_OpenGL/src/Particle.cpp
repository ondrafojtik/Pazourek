...
Particle::Particle(float xPos, float yPos, float width, float height, glm::vec4 startingColor, glm::vec4 dyingColor, float life)
	: m_xPos(xPos), m_yPos(yPos), m_Width(width), m_Height(height), m_startingColor(startingColor), m_dyingColor(dyingColor), m_life(life), m_currLife(life), m_Color(startingColor)
{
	m_Sprite = new Sprite(m_xPos, m_yPos, m_Width, m_Height, m_Color, "src/res/shaders/Sphere.shader");
...
{
	Particle(xPos, yPos, width, height, startingColor, dyingColor, 100.0f);
}
